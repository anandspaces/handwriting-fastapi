...
# Compatibility functions for TensorFlow 2.x
def _concat(prefix, suffix, static=False):
    """Concatenate prefix and suffix for creating shapes."""
    # Convert to lists if needed
    if hasattr(prefix, '__iter__') and not isinstance(prefix, (tf.Tensor, tf.TensorShape)):
        prefix_list = list(prefix)
    elif isinstance(prefix, tf.TensorShape):
        prefix_list = prefix.as_list()
    elif isinstance(prefix, tf.Tensor):
        if static:
            prefix_list = prefix.shape.as_list()
        else:
            return array_ops.concat([[prefix], suffix if hasattr(suffix, '__iter__') else [suffix]], 0)
    else:
        prefix_list = [prefix]
    
    if hasattr(suffix, '__iter__') and not isinstance(suffix, (tf.Tensor, tf.TensorShape)):
        suffix_list = list(suffix)
    elif isinstance(suffix, tf.TensorShape):
        suffix_list = suffix.as_list()
    elif isinstance(suffix, tf.Tensor):
        if static:
            suffix_list = suffix.shape.as_list()
        else:
            return array_ops.concat([prefix_list, [suffix]], 0)
    else:
        suffix_list = [suffix]
    
    result = prefix_list + suffix_list
    return result if static else tf.constant(result)

def _like_rnncell(cell):
    """Check if cell has the required RNN cell interface."""
...
def raw_rnn(cell, loop_fn, parallel_iterations=None, swap_memory=False, scope=None):
    ...
    with vs.variable_scope(scope or "rnn") as varscope:
        # In TF2, check if we're NOT in eager mode (i.e., in graph mode)
